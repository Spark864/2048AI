...
cpdef push_board_right(board):
    cdef np.ndarray[np.int_t, ndim=2] new = np.zeros((CELL_COUNT, CELL_COUNT), dtype="int")
...
cpdef fixed_move(board):
    cdef object move_order = [move_left, move_up, move_down, move_right]
...
cpdef random_move(board):
    ...
    cdef int move_index
    cdef int score
    cdef object move_order = [move_right, move_up, move_down, move_left]
