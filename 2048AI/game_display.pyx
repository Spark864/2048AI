...
cdef int EDGE_LENGTH = 400
cdef int CELL_COUNT = 4
cdef int CELL_PAD = 10

cdef object UP_KEY = "'w'"
cdef object DOWN_KEY = "'s'"
cdef object LEFT_KEY = "'a'"
cdef object RIGHT_KEY= "'d'"
cdef object AI_KEY = "'q'"
cdef object AI_PLAY_KEY = "'p'"

cdef object LABEL_FONT = ("Verdana", 40, "bold")

cdef char* GAME_COLOR = "#a6bdbb"

cdef char* EMPTY_COLOR = "#8eaba8"

cdef object TILE_COLORS = {2: "#daeddf", 4: "#9ae3ae", 8: "#6ce68d", 16: "#42ed71",
                   32: "#17e650", 64: "#17c246", 128: "#149938",
                   256: "#107d2e", 512: "#0e6325", 1024: "#0b4a1c",
                   2048: "#031f0a", 4096: "#000000", 8192: "#000000",}

cdef object LABEL_COLORS = {2: "#011c08", 4: "#011c08", 8: "#011c08", 16: "#011c08",
...
class Display(Frame):
    ...
    def build_grid(self):
        cdef object background = Frame(self, bg=GAME_COLOR,
                           width=EDGE_LENGTH, height=EDGE_LENGTH)
        background.grid()
        cdef int row
        cdef object grid_row
        cdef object t
        cdef object cell
    ...
    def key_press(self, event):
        cdef bint valid_game = True
        cdef object key = repr(event.char)
        cdef int move_count
        cdef bint move_made
        if key == AI_PLAY_KEY:
            move_count = 0
            while valid_game:
                self.matrix, valid_game = game_ai.ai_move(self.matrix,44, 44)
                if valid_game:
                    self.matrix = game_functions.add_new_tile(self.matrix)
                    self.draw_grid_cells()
                move_count += 1
        if key == AI_KEY:
            self.matrix, move_made = game_ai.ai_move(self.matrix, 44, 44)
            if move_made:
                self.matrix = game_functions.add_new_tile(self.matrix)
                self.draw_grid_cells()
                move_made = False
